...
All API keys have to be added to `youte` before they can be used. `config add-key` stores your API key in a config file which `youte` will refer to when running queries.
...
The \<QUERY\> argument is compulsory, where you specify the search term to look for in YouTube video titles and descriptions.

The results of this command will be printed to the terminal. If you want to store the output in a file, enter the file name after the search query. The file must end in `.json` or `.jsonl`.

``` shell
$ youte search <QUERY> <OUTPUT>
```

Besides the above, there are several optional arguments that you can pass to further refine the search. To get a full list of available options, type:
