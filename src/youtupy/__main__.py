...
@click.option("-c", "--by-channel",
              help="Get all comments for a channel ID",
              is_flag=True)
@click.option("-v", "--by-video",
              help="Get all comments for a video ID",
              is_flag=True)
@click.option("--max-quota", default=10000,
              help="Maximum quota allowed",
              show_default=True)
@click.option("--name", help="Name of the API key (optional)")
def list_comments(filepath: str,
                  output: str,
                  max_quota: int,
                  name: str,
                  by_video,
...
@click.option("--channel",
              help="Hydrate channel IDs instead of video IDs",
              is_flag=True)
@click.option("--name", help="Name of the API key (optional)")
...
def add_key():
    ...
    click.echo()
    click.echo(
        'Once you have an API key, run `youtupy configure add-key` to start.')
    ...
    click.echo('To set an API key as default, run `youtupy config set-default'
               ' <name-of-key>')
...
def list_keys():
    ...
    if not Path(get_config_path()).exists():
        ...
        click.echo()
        click.secho('All API keys are stored in %s' % get_config_path(),
                    fg='green',
                    bold=True)
        click.echo()
